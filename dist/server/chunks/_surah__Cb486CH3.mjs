...
import { z as attr_class, y as attr, C as escape_html, B as ensure_array_like, J as stringify, D as renderComponent, h as renderTemplate, m as maybeRenderHead, g as addAttribute } from "./worker-entry_C-jIstQk.mjs";
...
function SurahReader($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<div class="flex flex-col gap-4"><audio preload="none"></audio> <div class="flex flex-wrap items-center gap-2 sticky top-14 bg-canvas/95 backdrop-blur-sm py-3 z-10 border-b-4 border-ink mb-2"><div class="flex border-2 border-ink shadow-[2px_2px_0px_0px_#0D0D0D]"><button${attr_class(`px-3 py-1.5 text-[10px] font-black uppercase tracking-wider transition-colors duration-75 ${"bg-ink text-white"}`)}>Standard</button> <button${attr_class(`px-3 py-1.5 text-[10px] font-black uppercase tracking-wider border-l-2 border-ink transition-colors duration-75 ${"bg-white text-ink hover:bg-canvas"}`)}>Perkataan</button></div> <button${attr_class(`chip-brutal ${"bg-accent-blue text-white"} transition-colors`)}>${escape_html("✓ Terjemahan")}</button> <button${attr_class(`chip-brutal ${"bg-white"} transition-colors`)}>${escape_html("Rumi")}</button> `);
    QuranSettings($$renderer2);
    $$renderer2.push(`<!----> <span class="text-[10px] font-black uppercase tracking-wider text-ink/40 ml-auto">`);
    if (isShortSurah) {
      $$renderer2.push("<!--[0-->");
      $$renderer2.push(`${escape_html(verses.length)} ayat`);
    } else {
      $$renderer2.push("<!--[-1-->");
      $$renderer2.push(`Halaman ${escape_html(currentPage)}/${escape_html(totalPages)}`);
    }
    $$renderer2.push(`<!--]--></span></div> `);
    if (loading) {
      $$renderer2.push("<!--[0-->");
      $$renderer2.push(`<div class="flex items-center justify-center py-16"><div class="card-brutal-sm text-center"><p class="font-black text-ink/40 uppercase tracking-wider animate-pulse">Memuatkan ayat-ayat...</p></div></div>`);
    } else if (error) {
      $$renderer2.push("<!--[1-->");
      $$renderer2.push(`<div class="card-brutal-sm bg-accent-pink text-white text-center"><p class="font-bold text-sm">${escape_html(error)}</p> <button class="btn-brutal-sm mt-2 bg-white text-ink text-xs">Cuba Lagi</button></div>`);
    } else {
      $$renderer2.push("<!--[-1-->");
      ...
      for (let $$index_1 = 0, $$length = each_array.length; $$index_1 < $$length; $$index_1++) {
        ...
        $$renderer2.push(`<div${attr_class(`verse-card ${isPlaying(verse.number) ? "verse-playing" : ""}`)} role="article"${attr("aria-label", `Verse ${stringify(verse.number)}`)}><div class="flex items-center gap-2 mb-3 border-b-2 border-ink/10 pb-2"><span class="w-8 h-8 bg-ink text-white flex items-center justify-center font-black text-xs shrink-0">${escape_html(verse.number)}</span> `);
        if (verse.audio?.url) {
          $$renderer2.push("<!--[0-->");
          $$renderer2.push(`<button${attr_class(`w-8 h-8 border-2 border-ink flex items-center justify-center shrink-0 transition-all duration-75 active:translate-x-0.5 active:translate-y-0.5 active:shadow-none ${isPlaying(verse.number) ? "bg-accent-yellow shadow-[2px_2px_0px_0px_#0D0D0D]" : "bg-white hover:bg-canvas"}`)}${attr("aria-label", isPlaying(verse.number) ? `Pause verse ${verse.number}` : `Play verse ${verse.number}`)}>${html(isPlaying(verse.number) ? pauseIcon : playIcon)}</button>`);
        } else {
          $$renderer2.push("<!--[-1-->");
        }
        $$renderer2.push(`<!--]--> `);
...
const $$surah = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "BaseLayout", $$BaseLayout, { "title": `${surah.englishName} — Al-Quran` }, { "default": ($$result2) => renderTemplate` ${maybeRenderHead()}<div class="max-w-4xl mx-auto px-4 py-8 pb-32"> <!-- Navigation --> <div class="flex items-center justify-between mb-4"> <a href="/quran" class="btn-brutal-sm text-sm">← Semua Surah</a> <div class="flex items-center gap-2"> ${num > 1 && renderTemplate`<a${addAttribute(`/quran/${num - 1}`, "href")} class="btn-brutal-sm text-sm" title="Surah sebelumnya">← ${SURAHS[num - 2]?.englishName ?? "Prev"}</a>`} ${num < 114 && renderTemplate`<a${addAttribute(`/quran/${num + 1}`, "href")} class="btn-brutal-sm text-sm" title="Surah seterusnya">${SURAHS[num]?.englishName ?? "Next"} →</a>`} </div> </div> <!-- Surah Hero Header --> <div class="card-hero mb-6 text-center relative overflow-hidden"> <!-- Revelation type badge --> <div class="absolute top-4 right-4"> <span class="chip-brutal {surah.revelationType === 'Meccan' ? 'bg-accent-yellow' : 'bg-accent-blue text-white'}"> ${surah.revelationType === "Meccan" ? "🕋 Makkah" : "🕌 Madinah"} </span> </div> <div class="flex flex-col items-center gap-3"> <!-- Surah number --> <div class="w-16 h-16 bg-ink text-white flex items-center justify-center font-black text-2xl shadow-[4px_4px_0px_0px_#FFD500]"> ${num} </div> <div> <h1 class="text-3xl font-black uppercase tracking-wider">${surah.englishName}</h1> <p class="text-sm text-ink/60 font-bold mt-1">${surah.englishMeaning} · ${surah.versesCount} ayat</p> </div> <!-- Arabic name --> <p class="arabic-text text-3xl" dir="rtl" style="line-height: 1.8;">${surah.name}</p> </div> <!-- Bismillah --> ${num !== 1 && num !== 9 && renderTemplate`<div class="mt-6 pt-4 border-t-4 border-ink/10"> <p class="arabic-text text-xl" dir="rtl">بِسْمِ ٱللَّهِ ٱلرَّحْمَـٰنِ ٱلرَّحِيمِ</p> <p class="text-xs text-ink/40 font-bold mt-1">Dengan nama Allah, Yang Maha Pemurah, lagi Maha Mengasihani</p> </div>`} </div> <!-- Surah Reader (Svelte 5 island) --> ${renderComponent($$result2, "SurahReader", SurahReader, { "chapter": num, "client:load": true, "client:component-hydration": "load", "client:component-path": "C:/Users/alif325/Documents/WIndsurf projeks/Ruang/src/components/quran/SurahReader.svelte", "client:component-export": "default" }, { "default": ($$result3) => renderTemplate` <div class="text-center py-16 animate-pulse"> <p class="font-black text-ink/40 uppercase tracking-wider">Memuatkan ayat-ayat...</p> </div> ` })} </div> ` })}`;
